...
import com.example.tickettrackingsystem.dao.UserDao;
...
public class UserServiceImpl implements UserDetailsService, UserService {
    ...
    public User save(UserDto user) {
        ...
        String postfix = nUser.getEmail().split("@")[1];
        switch (postfix)
        {
            case "admin.ruckus":
                ...
                break;
            case "qa.ruckus":
                role = roleService.findByName("QA");
                roleSet.add(role);
                break;
            case "rd.ruckus":
                role = roleService.findByName("RD");
                roleSet.add(role);
                break;
            case "pm.ruckus":
                role = roleService.findByName("PM");
                roleSet.add(role);
                break;
            default:
                System.out.println("User only have default USER ROLE");
        }
